...
simGenerator <- function(ampli, period, smooth, temp0){ #generate different temperature and Rt data set to apply to epifilter
...
for(row in 1:nrow(vars)){
  params[[row]] <- simGenerator(vars$ampli[row], vars$period[row], vars$smooth[row], vars$temp[row])
}

save(params, file="./out/par.Rdata")
load("./out/par.Rdata")

#plot true Rt to check
# extract <- function(var){unlist(lapply(params, "[[", var))}
# df <- data.frame(ampli=extract("Amp"), period=extract("Period"), Rt=extract("Rt"))
# plotlist <- list()
# for(i in 1:length(params)){
#   plotlist[[i]] <- local({
#     i <- i
#     ggplot() +
#       geom_line(aes(x=1:200, y=params[[i]][["Rt"]])) +
#       labs(x="", y="Rt", title=paste("Ampli:", params[[i]][["Amp"]], "; Period:", params[[i]][["Period"]]))
#   })
# }

# pdf(file="./plots/trueRt_smooth.pdf", paper="a4r", width=11, height=8)
# plot_grid(plotlist=plotlist[1:9], nrow=3, ncol=3)
# dev.off()
# 
# pdf(file="./plots/trueRt_noisy.pdf", paper="a4r", width=11, height=8)
# plot_grid(plotlist=plotlist[10:18], nrow=3, ncol=3)
# dev.off()
...
for(index in 1:length(params)){
  ...
  result[[index]] <- list("Omsmooth"=Omsmooth, "Rsmooth"=Rsmooth, "Rsmooth_td"=Rsmooth_td 
              ,"IsmoothOm"=IsmoothOm, "Ismooth"=Ismooth, "Ismooth_td"=Ismooth_td)
}
...
#   real <- params[[i]][["True Inc"]][2:200]
#   real[which(real==0)] <- 1
#   mape_ti <- mean(abs(real - result[[i]][["Ismooth"]][["pred"]])/real)
#   mape_td <- mean(abs(real - result[[i]][["Ismooth_td"]][["pred"]])/real)
#   mape_om <- mean(abs(real- result[[i]][["IsmoothOm"]][["pred"]])/real)
#   mape_list[[i]] <- c(mape_ti, mape_td, mape_om)
# }

save(list=c("vars", "params", "result"), file="./sim_output.Rdata")
